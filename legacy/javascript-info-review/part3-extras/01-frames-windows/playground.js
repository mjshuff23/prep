...
function writePopupDocument(target) {
...
function wirePopup(target) {
  ...
  reply.addEventListener("click", (event) => {
    ...
    sourceWindow.opener?.postMessage({ kind: "child-reply", at: Date.now() }, "*");
  });

  target.opener?.postMessage({ kind: "child-ready" }, "*");
}
